이 책은 25가지 SQL 안티패턴들을 소개해준다.
이 패턴들을 소개해주면서 대신 쓸수 있는 방법들(Best Practice)를 알려주기 때문에 좋은 책이다.

일반적인 SQL 문법을 소개하거나
데이터베이스론 같은 학술적 접근이 아니라
실제 업무 환경에서 사용할 수 있는 무언가를 가르쳐주기 때문에 좋은 책이다.

하지만 너무 상식적인 것도 안티패턴으로 소개하고 있기 때문에
그런 경우에는 자세히 쓰지 않는다.
...
서브타입모델링 방법1 --> 단일 테이블 상속
두 서브타입(Bugs, FeatureRequest) 의 모든 속성을 담을 수 있는 공통테이블 Issues를 만든다.
이 테이블에는 두 테이블에서 필요한 모든 컬럼이 선언되어 데이터가 insert 된다.
어떤 row가 두 테이블 중 어떤 테이블에 속한것인지를 표시한는 issue_type 컬럼이 존재한다.(BUG 또는 FEATURE 라는 값으로 구분)
그 외에 컬럼에는 공통적으로 사용하는 컬럼들과 각 테이블에서만 사용하는 컬럼들도 존재한다.

서브타입모델링 방법2 -- 구체 테이블 상속
가장 단순하게 두 서브타입별로 테이블을 만드는 것이다.
공통 속성도 각자 갖고 있으며 각 테이블에서만 사용하는 컬럼은 각각 갖고 있다.

서브타입모델링 방법3 -- 클래스 테이블 상속
서브타입에서 공통적인 속성을 Issues 테이블에 선언하고
각 서브타입별 컬럼들은 각자의 테이블에 유지한다.
Issues 테이블과 Bugs, FeatureRequests 테이블은 issue_id로 FK로 연결하여 일대일 관계를 강제한다.

서브타입모델링 방법4 -- Semistructured Data
서브타입의 수가 많을 경우 JSON같은 형식으로 코딩하여 TEXT 또는 CLOB 컬럼으로 저장한다.
공통적인 속성을 그대로 Issues 테이블에 컬럼으로 정의하고,
서브타입별 컬럼을 attribute 컬럼( TEXT, CLOB ) 타입으로 정의해 JSON 같은 형식으로 저장하는 형태이다.
어떤 row가 두 테이블 중 어떤 테이블에 속한것인지를 표시한는 issue_type 컬럼이 존재한다.(BUG 또는 FEATURE 라는 값으로 구분)
...
즉, Bugs 와 FeatureReqeust는 Comment라는 부모를 가진다.

목표는 여러가지 부모를 참조하는 것이다.

이 경우를 해결하는 안티패턴은
기존 comment 테이블에 issue type 컬럼을 만들고 값을 bugs 또는 featureReqeust 로 표시하고
bugs 전용 comment 테이블이었다면 가져야 했을 fk 인 bugs id 대신 두 테이블의 id와 연결되는 issue id를 사용한다.
그러나 두 테이블에 fk를 강제할 수 없으므로 bugs 전용 테이블이었을 때와 다르게 issue_id는 fk가 될 수 없다.
...
소프트웨어 회사에서 버그를 관리하는 테이블이며 BugsProduct는 제품(Products) 테이블과 다대다 관계 해소를 위한 테이블과 bug_id로 조인하는 SQL이다
GROUP BY 기준 컬럼은 product_id이며 MAX 함수를 보고된 날짜(date_reported) 컬럼에 적용했다.
그리고 bug_id 컬럼을 추가적으로 출력하도록 SELECT LIST 절에 포함했다.

문제 1 date_reported값이 동일했을 때 그룹내 최댓값이라면 쿼리에서 어떤 bug_id 값을 보여줘야 하는가?
...
의문 2. 두 종류의 집계 함수인 MAX, MIN 함수를 아래와 같이 사용한다면 그룹 안에서 두개와 다른 행에 대응되는데, 이때는 각 그룹에서 어떤 bug_id를 리턴해야 하는가?
...
의문 3 집계 함수가 리턴하는 값과 매치되는 행이 없는 경우에는 bug id 값을 어떻게 해야 하는가?
...
상관 서브쿼리는 바깥쪽 쿼리의 각 행에 대해 한번씩 실행되므로 성능상 최적의 방법은 아니다.
...
유도 테이블은 상관 서브쿼리보다 규모가변성(scalability) 가 좋은 대안이다.
상호 연관되지 않으므로 대부분의 DB에서 서브쿼리가 한번만 수행되지만
임시 테이블에 중간 결과를 저장해야 하므로 성능상 최적의 방법은 아니다.
(임시 테이블 중간 결과 저장에 대한 역주 --> 실행 계획이 조인으로 풀릴 수(변환) 있으므로 반드시 중간 결과가 임시테이블에 저장되는 것은 아니다.)
...
### 패턴 22 가상키 편집증( 체계적이고 논리적인 망상을 지속적으로 고집하는 병적 상태 )
...
안티패턴은 DB 코드를 애플리케이션과 동일하지 않은 절차로 처리하는 것(SQL을 2등 시민으로 만든다) 이다.
...
코드 해독하는데 문서화하지 않는 것보다 인내와 에너지, 시간을 더 요구한다. 또한 코드는 빠진 기능이나 해결하지 못한 문제는 알려주지 않는다.
...
뷰 또한 마찬가지다. 뷰를 생성해야 하는 이유는 무엇인지(뷰가 테이블의 복잡한 관계를 추상화하려는 의도인지, 권한이 없는 사용자를 위해 일부 데이터만 노출하기 위한 것인지 등)
...
API 처럼 문서화해야 한다.
프로시저가 어떤 문제를 해결하기 위해 작성되었는지, 입출력 파라미터의 데이터 타입과 그 의미
...
- 테이블, 컬럼, 뷰  
   테이블과 뷰가 실제로 존재하는지 테이블, 뷰, 컬럼을 추가할때마다 객체가 있는지를 확인하는 테스트를 추가한다.(잘 추가 되었는지 확인하는 테스트를 의미하는 듯 하다.)
