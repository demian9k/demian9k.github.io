...
배스킨라빈스은 1953년 창업하여, 몇년동안 31가지의 맛을 제공한다는 슬로건을 사용했다.

수십년 후 배스킨라빈스는 클래식 메뉴, 계절 메뉴, 지역별 메뉴, 아이스크림별 선택 가능한 옵션 사항 등
다양한 구성으로 아이스크림을 맛볼수 있게 운영하고 있다.

저자는 이 이야기를 들려주며 데이터베이스 설계할때도 처음에 충분하다고 생각했던 어떤 조건들이 확장성이 필요해질 수 있는 경우가 있음을 알기를 바란다.

아래 테이블은 연락처 정보 테이블이며
인사말 컬럼에 호칭 접두사 5가지만 입력될 수 있도록 check constraint이 적용되었다.
...
아래 테이블은 Bugs 테이블이다.
salutation 과 같이 bugs 테이블의 status 컬럼이 변화할 수 있는 경우를 해결한 설계이다.
...
BugStatus 테이블을 별도로 만들어 가능한 값을 insert 한 뒤 Bugs 테이블에서 FK로 해당 BugStatus테이블의 status를 FK로 지정하였다.

값이 변할 경우 단순히 bugstatus 테이블에 status 값을 insert 하거나 delete 하면 서비스에 영향을 주지 않고 Bugs 테이블에서 status값을 check constraint 처럼 제한할 수 있다.
...
### 패턴 22 가상키 편집증 Pseudokey Neat-Freak (한글로는 편집증, 영어는 결벽증)
