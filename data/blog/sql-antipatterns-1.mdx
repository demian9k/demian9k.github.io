...
```sql:first_comments_table {3-4, 8}
CREATE TABLE Comments (
    comment_id SERIAL PRIMARY KEY,
    bug_id BIGINT UNSIGNED NOT NULL,
...
```sql:ANTIPATTERN-dual_purpose_FK {3-4}
CREATE TABLE Comments (
    comment_id SERIAL PRIMARY KEY,
...
```sql:SOLUTION1_create_intersection_table {}
...
```sql:SOLUTION1_1_add_unique_constraint_on_commentId {4}
CREATE TABLE BugsComments (
    issue_id BIGINT UNSIGNED NOT NULL,
    comment_id BIGINT UNSIGNED NOT NULL,
...
```sql:SOLUTION2_create_Common-Super-Table {2, 6, 12, 19}
CREATE TABLE Issues (
    issue_id SERIAL PRIMARY KEY
);
...
```sql:solution2_vertical_partitioning
